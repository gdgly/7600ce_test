...
{
 	int fd;
	fd = open("/sys/class/spi_master/spi2/device/power/control", O_RDWR);
	if (fd < 0){
...
int spi_try_dev_mount(char* devpath)
{
	if(0 != access(devpath,F_OK)){
		unsigned char busnum,chipselect;
		char mountcmd[128] = {0};
		...
		sprintf(mountcmd,"insmod /usr/lib/modules/3.18.20/kernel/drivers/spi/spidev.ko busnum=%d chipselect=%d",busnum,chipselect);
		printf("%s\n", mountcmd);
		system(mountcmd);
		usleep(50000);
	}
...
//	parse_opts(argc, argv);
	...
	if(0 != spi_try_dev_mount((char* )device)){
