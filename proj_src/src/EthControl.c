...
int Install_driver()
{
    ...
    sprintf(command, "%s", "insmod /usr/lib/modules/3.18.20/kernel/drivers/net/phy/libphy.ko"); 
    ds_system_call(command, strlen(command));

    memset(command, 0, sizeof(command));
    sprintf(command, "%s", "insmod /usr/lib/modules/3.18.20/kernel/drivers/of/of_mdio.ko"); 
    ds_system_call(command, strlen(command));

    if(eth_type == ETH_TYPE_BCM898XX)
    {
        memset(command, 0, sizeof(command));
        sprintf(command, "%s", "insmod /modules/3.18.20/kernel/drivers/net/phy/bcm898xx.ko"); 
        ds_system_call(command, strlen(command));
    }
    else if(eth_type == ETH_TYPE_AT803X)
    {
        memset(command, 0, sizeof(command));
        sprintf(command, "%s", "insmod /modules/3.18.20/kernel/drivers/net/phy/at803x.ko"); 
        ds_system_call(command, strlen(command));
    }

    memset(command, 0, sizeof(command));
    sprintf(command, "%s", "insmod /usr/lib/modules/3.18.20/kernel/drivers/net/phy/qca8337.ko"); 
    ds_system_call(command, strlen(command));

    memset(command, 0, sizeof(command));
    sprintf(command, "%s", "insmod /usr/lib/modules/3.18.20/kernel/drivers/net/ethernet/qualcomm/emac/qcom_emac.ko"); 
    ds_system_call(command, strlen(command)); 
