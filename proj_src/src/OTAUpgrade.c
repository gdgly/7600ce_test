...
* Date          : 2018.04.14
*****************************************************************************/
int ota_downloadFile(char *fileName, char *destination)
{
    int Pasvfd;
    int PasvPort;
    char PasvIp[32];
    ...
    ftp_init((char *)FTP_IP, (int)FTP_PORT);
    //FTPClient ftpClient("FTP_IP", "FTP_PORT");
    if(loginFTPServer((char *)"FTP_UsrName", (char *)"FTP_Password") == -1)
        return -1;
